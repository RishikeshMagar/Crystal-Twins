...
class Supervise(object):
    ...
    def train(self):
        ...
        if self.config['optim']['optimizer'] == 'SGD':
            ...
            optimizer = optim.Adam(
                model.parameters(), self.config['optim']['lr'], 
                weight_decay=eval(self.config['optim']['weight_decay'])
            )
        else:
    ...
    def _load_pre_trained_weights(self, model):
        ...
        try:
            checkpoints_folder = os.path.join('./runs', self.config['fine_tune_from'], 'checkpoints')
            checkpoint = torch.load(os.path.join(checkpoints_folder, 'model.pth'),  map_location=self.config['gpu'])  
...
if __name__ == "__main__":
    ...
    if 'BG' in config['dataset']['root_dir']:
        config['task'] = 'regression'
        task_name = 'BG'
    elif 'E_form' in config['dataset']['root_dir']:
        config['task'] = 'regression'
        task_name = 'E_form'
    elif 'Is_Metal' in config['dataset']['root_dir']:
        config['task'] = 'classification'
        task_name = 'Is_Metal'
    elif 'MP-formation-energy' in config['dataset']['root_dir']:
        config['task'] = 'regression'
        task_name = 'MP_energy'
    elif 'dielectric' in config['dataset']['root_dir']:
        config['task'] = 'regression'
    ...
    supervise.train()
    loss, metric = supervise.test()

    import pandas as pd
    df = pd.DataFrame([[loss, metric.item()]])
    df.to_csv('experiments/{}_sup.csv'.format(task_name), 
